...
//   int32_t duration_ms = (end_tick - start_tick) * portTICK_PERIOD_MS;

if (invoke_status != kTfLiteOk) {
    MicroPrintf("Invoke failed");
    return;
}
//  int durationsec_ms = (end_time - start_time) / 1000;

//   MicroPrintf("Inference Latency: %d ms", durationsec_ms);
// // 4. Print it
//   MicroPrintf("Inference Latency with ticqs: %d ms", duration_ms);
